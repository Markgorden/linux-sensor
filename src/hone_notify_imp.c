...
static DEFINE_STATISTICS(hone_received); /* Events from the kernel we received. */
static DEFINE_STATISTICS(hone_dropped);  /* Events from the kernel we dropped. */

struct workqueue_struct *hone_wq;
...
static void copy_statistics(const struct statistics *src, struct statistics *dst)
{
	...
	copy_atomic64(dst->aggregate, src->aggregate);
}
...
int hone_notifier_unregister(struct notifier_block *nb)
{
	...
	result = atomic_notifier_chain_unregister(&notifier_list, nb);
	if (notifier_call_chain_empty())
...
{
	return atomic_notifier_call_chain(&notifier_list, 0, event);
}
...
void free_hone_event(struct hone_event *event)
{
	if (event->type == HONE_PACKET) {
		...
		if (event->process.mm) {
			...
			BUG_ON(unlikely(!queue_work(hone_wq, (struct work_struct*)delayed_mm)));
		}
		if (event->process.cwd)
			kfree(event->process.cwd);
	}
		...
		struct process_args *pargs, int type, gfp_t flags)
{
	...
	if ((event = alloc_hone_event(HONE_PROCESS, flags))) {
		struct task_struct *task = pargs->task;
		struct process_event *pev = &event->process;
		const struct cred *cred;
		if (type == PROC_EXIT) {
			pev->code = pargs->code;
			pev->signaled = task->flags & PF_SIGNALED;
		} else if (type == PROC_EXEC) {
			if (task->fs) {
				char *c, *cwd;
				struct path path;

				cwd = kmalloc(PAGE_SIZE, GFP_ATOMIC);
				if (!cwd)
					goto cwd_failed;

				get_fs_pwd(task->fs, &path);
				c = d_path(&path, cwd, PAGE_SIZE);
				if (IS_ERR_OR_NULL(c))
					goto cwd_failed;

				pev->cwd = kstrdup(c, GFP_ATOMIC);
cwd_failed:
				path_put(&path);
				kfree(cwd);
			}
		...
		if (unlikely(!cred)) {
			printk(KERN_ERR "__alloc_process_event: cred is null\n");
			pev->uid = -1;
			pev->euid = -1;
			pev->gid = -1;
		} else {
		...
		struct task_struct *task, gfp_t flags)
{
	...
	if ((event = alloc_hone_event(HONE_SOCKET, flags))) {
		...
		cred = __task_cred(task);
		if (unlikely(!cred)) {
			printk(KERN_ERR "__alloc_socket_event: cred is null\n");
			sockev->uid = -1;
			sockev->gid = -1;
		} else {
...
_STATIC int __init hone_notify_init(void)
{
	...
	hone_wq = create_singlethread_workqueue("hone_wq");
	if (!hone_wq) {
		err = -1;
		goto out_workqueue;
	}
